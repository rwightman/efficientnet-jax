...
from .block_defs import round_channels


__all__ = ['EfficientNetBuilder', 'decode_arch_def']
...
def _decode_block_str(block_str):
    ...
    if block_type == 'ir':
        ...
        if 'cc' in options:
            assert False, "CondConv not currently impl"
    ...
    def __call__(self):
        ...
        for stack_idx, stack_args in enumerate(self.block_args):
            ...
            for block_idx, block_args in enumerate(stack_args):
                ...
                # if extract_features:
                #     feature_info = dict(stage=stack_idx + 1, reduction=current_stride)
                #     # FIXME support block specific feature names?, **block.feature_info(self.feature_location))
