...
class _ShampooDefaultParamState:
    ...
    diagonal_statistics: np.ndarray
    # Statistics
    statistics: np.ndarray
    # Preconditioners
    preconditioners: np.ndarray
    # Momentum for the diagonal preconditioner
    diagonal_momentum: np.ndarray
...
def power_iter(mat_g, error_tolerance=1e-6, num_iters=100):
    ...
    v_0 = np.random.uniform(-1.0, 1.0, mat_g_size).astype(mat_g.dtype)

    init_state = tuple([0, v_0, jnp.zeros([], dtype=mat_g.dtype), v_0, True])
    num_iters, v_out, s_out, _, _ = lax.while_loop(_iter_condition, _iter_body, init_state)
...
def matrix_inverse_pth_root(mat_g,
    ...
    def _iter_body(state):
        ...
        new_mat_m = jnp.matmul(mat_power(mat_m_i, p), mat_m, precision=_INVERSE_PTH_ROOT_PRECISION)
...
class Shampoo(OptimizerDef):
    ...
    def __init__(self,
        ...
        Args:
            ...
            batch_axis_name: labeled axis over pmap for dataparallel training the optimizer used for.
        """
    ...
    def fast_cond(self, predicate, compute_fn, init_state, *args, **kwargs):
        ...
        results = lax.while_loop(_iter_condition, _iter_body, tuple([predicate] + init_state))
    ...
    def compute_shampoo_statistics(self, step, hps, param, state, grad):
        ...
        if not self._skip_preconditioning(param, hps):
            ...
            if hps.statistics_compute_steps > 1:
                ...
                new_statistics = list(
                    self.fast_cond(perform_step, compute_updated_statistics, init_state))
            else:
    ...
    def compute_preconditioners_from_statistics(self, states, hps, step):
        ...
        for state in states:
            ...
            if num_statistics > 0:
                for statistic in state.statistics:
                    exponents.append(2 * num_statistics if hps.exponent_override == 0
                                     else hps.exponent_override)
        ...
        all_statistics, all_exponents = _batch(packed_statistics, exponents, num_devices)
        ...
        if not hps.batch_axis_name:
            preconditioners, errors = jax.pmap(_matrix_inverse_pth_root)(all_statistics, all_exponents)
            ...
            def _internal_inverse_pth_root_all():
                ...
                preconditioners = jax.lax.all_gather(preconditioners, hps.batch_axis_name)
            ...
            if hps.preconditioning_compute_steps == 1:
                ...
                errors_init = ([_INVERSE_PTH_ROOT_FAILURE_THRESHOLD] * len(packed_statistics))
    ...
    def apply_per_param_gradient(self, step, hps, param, state, grad):
        ...
        if hps.graft_type == LayerwiseGrafting.ADAGRAD:
            ...
            adagrad_update = grad / (jnp.sqrt(new_diagonal_statistics) + hps.diagonal_eps)
        ...
        shampoo_update = precond_grad * (grafting_update_norm / (precond_grad_norm + 1e-16))
        ...
        grafting_update_with_wd_momentum = state.diagonal_momentum * hps.beta1 + grafting_update_with_wd
